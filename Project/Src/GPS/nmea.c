...
gnss_ret_e nmea_processNMEA(gnss_data_t *data, uint8_t *line, uint16_t sz, gnss_nmea_driver_t *driver)
{
    ...
    if ((ret = nmea_verifyChecksum(line, sz)) == GNSS_SUCCESS)
    {
        ...
        if (line[1] == 'G' && (line[2] == 'P' || line[2] == 'N' || line[2] == 'L' || line[2] == 'A'))
        {
            ...
            {
            case 'P':
                driver->currentMessage |= NMEA_GP;
                break; // GPS
            case 'N':
                driver->currentMessage |= NMEA_GN;
                break; // Multiple GPS + GLONASS
            case 'L':
                driver->currentMessage |= NMEA_GL;
                break; // Glonas
            case 'A':
                driver->currentMessage |= NMEA_GA;
                break; // Galileo
            default:
                break;
            }
            ...
            if (sz > 6 && line[3] == 'R' && line[4] == 'M' && line[5] == 'C')
            {
                ...
                if (nmea_goNextField(&pt) != GNSS_SUCCESS)
                    return GNSS_INVALIDFORMAT;
                uint16_t speed;
                if (nmea_getRationalField(pt, &speed) == GNSS_INVALIDFORMAT)
                    return GNSS_INVALIDFORMAT;

                if (nmea_goNextField(&pt) != GNSS_SUCCESS)
                    return GNSS_INVALIDFORMAT;
                uint16_t cog;
                if (nmea_getRationalField(pt, &cog) == GNSS_INVALIDFORMAT)
                    return GNSS_INVALIDFORMAT;
                ...
                {
                case 'A':
                    data->fixInfo.positionMode = GNSS_POSMODE_AUTONOMOUS;
                    break;
                case 'D':
                    data->fixInfo.positionMode = GNSS_POSMODE_DIFFERENTIAL;
                    break;
                case 'E':
                    data->fixInfo.positionMode = GNSS_POSMODE_ESTIMATED;
                    break;
                case 'M':
                    data->fixInfo.positionMode = GNSS_POSMODE_MANUAL;
                    break;
                case 'S':
                    data->fixInfo.positionMode = GNSS_POSMODE_SIMULATOR;
                    break;
                case 'N':
                    data->fixInfo.positionMode = GNSS_POSMODE_INVALID;
                    break;
                default:
                    data->fixInfo.positionMode = GNSS_POSMODE_UNKNOWN;
                    break;
                }
                // process dateTime
                ...
                // process position
                int32_t ilat = 0;
                if (nmea_getLatLngField(lat, &ilat, ns) != GNSS_SUCCESS)
                    return GNSS_INVALIDFORMAT;
                int32_t ilon = 0;
                if (nmea_getLatLngField(lon, &ilon, ew) != GNSS_SUCCESS)
                    return GNSS_INVALIDFORMAT;
            ...
            else if (sz > 6 && line[3] == 'G' && line[4] == 'G' && line[5] == 'A')
            {
                ...
                if (*pt != ',')
                {
                    ...
                    if (nmea_goNextField(&pt) != GNSS_SUCCESS)
                        return GNSS_INVALIDFORMAT;
                    uint16_t nSatUsed;
                    if (nmea_getDecimalField(pt, &nSatUsed) == GNSS_INVALIDFORMAT)
                        return GNSS_INVALIDFORMAT;
                    if (nmea_goNextField(&pt) != GNSS_SUCCESS)
                        return GNSS_INVALIDFORMAT;
                    uint16_t hdop;
                    if (nmea_getRationalField(pt, &hdop) == GNSS_INVALIDFORMAT)
                        return GNSS_INVALIDFORMAT;
                    ...
                    if (*pt != ',')
                    {
                        if (nmea_getDecimalField(pt, &altitude) == GNSS_INVALIDFORMAT)
                            return GNSS_INVALIDFORMAT;
                        fix3d = true;
                    }
                    ...
                    if (nmea_getRationalField(pt, &hgeoid) == GNSS_INVALIDFORMAT)
                        return GNSS_INVALIDFORMAT;
            ...
            else if (sz > 6 && line[3] == 'G' && line[4] == 'L' && line[5] == 'L')
            {
                ...
                if (*time != ',')
                {
                    ...
                    {
                        itsdk_time_sync_UTC_s(data->gpsTime.hours * 3600 + data->gpsTime.minutes * 60 + data->gpsTime.seconds);
                        if (data->fixInfo.fixType < GNSS_FIX_TIME)
                            data->fixInfo.fixType = GNSS_FIX_TIME;
                    }
                    itsdk_time_is_UTC_s(&data->lastRefreshS);
            ...
            else if (sz > 6 && line[3] == 'G' && line[4] == 'S' && line[5] == 'A')
            {
                ...
                uint16_t pdop;
                if (nmea_getRationalField(pt, &pdop) == GNSS_INVALIDFORMAT)
                    return GNSS_INVALIDFORMAT;
                uint16_t hdop;
                if (nmea_goNextField(&pt) != GNSS_SUCCESS)
                    return GNSS_INVALIDFORMAT;
                ...
                if (nmea_goNextField(&pt) != GNSS_SUCCESS)
                    return GNSS_INVALIDFORMAT;
                if (nmea_getRationalField(pt, &vdop) == GNSS_INVALIDFORMAT)
                    return GNSS_INVALIDFORMAT;
...
#error "At least one of the gnss shall be enabled"
            ...
            else if (sz > 6 && line[3] == 'G' && line[4] == 'S' && line[5] == 'V')
            {
                ...
                if (nmea_goNextField(&pt) != GNSS_SUCCESS)
                    return GNSS_INVALIDFORMAT;
                if (nmea_getDecimalField(pt, &totSentences) == GNSS_INVALIDFORMAT)
                    return GNSS_INVALIDFORMAT;
                if (nmea_goNextField(&pt) != GNSS_SUCCESS)
                    return GNSS_INVALIDFORMAT;
                if (nmea_getDecimalField(pt, &curSentence) == GNSS_INVALIDFORMAT)
                    return GNSS_INVALIDFORMAT;
                ...
                if (nmea_goNextField(&pt) != GNSS_SUCCESS)
                    return GNSS_INVALIDFORMAT;
                if (nmea_getDecimalField(pt, &totSatInView) == GNSS_INVALIDFORMAT)
                    return GNSS_INVALIDFORMAT;
                if (totSatInView > 0)
                {
                    ...
                    for (int i = 0; i < pendingSat; i++)
                    {
                        ...
                        if (nmea_goNextField(&pt) != GNSS_SUCCESS)
                            return GNSS_INVALIDFORMAT;
                        if (nmea_getDecimalField(pt, &satId) == GNSS_INVALIDFORMAT)
                            return GNSS_INVALIDFORMAT;
                        if (nmea_goNextField(&pt) != GNSS_SUCCESS)
                            return GNSS_INVALIDFORMAT;
                        if (nmea_getDecimalField(pt, &elevation) == GNSS_INVALIDFORMAT)
                            return GNSS_INVALIDFORMAT;
                        if (nmea_goNextField(&pt) != GNSS_SUCCESS)
                            return GNSS_INVALIDFORMAT;
                        if (nmea_getDecimalField(pt, &azimuth) == GNSS_INVALIDFORMAT)
                            return GNSS_INVALIDFORMAT;
                        if (nmea_goNextField(&pt) != GNSS_SUCCESS)
                            return GNSS_INVALIDFORMAT;
                        if (nmea_getDecimalField(pt, &snr) == GNSS_INVALIDFORMAT)
                            return GNSS_INVALIDFORMAT;
                        ...
                        if (satId >= 1 && satId <= 32)
                        {
                            ...
                            if (snr > data->sat_gps[satId - 1].maxSignal)
                                data->sat_gps[satId - 1].maxSignal = snr;
                        }
#endif
                        ...
                        if (satId >= 65 && satId <= 88)
                        {
                            ...
                            if (snr > data->sat_glonas[satId - 65].maxSignal)
                                data->sat_glonas[satId - 65].maxSignal = snr;
                        }
#endif
...
gnss_ret_e nmea_addChecksum(uint8_t *line, uint16_t sz)
{
    ...
    itdt_convertInt2HexChar(chksum, (char *)&line[chksumpos + 1], true);
    line[chksumpos + 3] = '\r';
    line[chksumpos + 4] = '\n';
    line[chksumpos + 5] = '\0';
    return GNSS_SUCCESS;
...
gnss_ret_e nmea_getLatLngField(uint8_t *l, int32_t *degrees, uint8_t orientation)
{
    ...
    else if (l[5] == '.')
    {
        ...
        return GNSS_INVALIDFORMAT;
    degree += 100000000 * __NMEA_CONVERT_CHAR(l[0 + shift]) + 10000000 * __NMEA_CONVERT_CHAR(l[1 + shift]);
    minute = 100000000 * __NMEA_CONVERT_CHAR(l[2 + shift]) + 10000000 * __NMEA_CONVERT_CHAR(l[3 + shift]) + 1000000 * __NMEA_CONVERT_CHAR(l[5 + shift]) + 100000 * __NMEA_CONVERT_CHAR(l[6 + shift]) + 10000 * __NMEA_CONVERT_CHAR(l[7 + shift]) + 1000 * __NMEA_CONVERT_CHAR(l[8 + shift]);
    degree += minute / 60;
