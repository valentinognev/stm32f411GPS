...
gnss_ret_e quectel_lxx_initLowPower(gnss_config_t *config)
{
    ...
    //         LL_GPIO_SetOutputPin(ITSDK_DRIVERS_GNSS_QUECTEL_L8X_POWERON_BANK,ITSDK_DRIVERS_GNSS_QUECTEL_L8X_POWERON_PIN);
    //     #else
    //         LL_GPIO_ResetOutputPin(ITSDK_DRIVERS_GNSS_QUECTEL_L8X_POWERON_BANK,ITSDK_DRIVERS_GNSS_QUECTEL_L8X_POWERON_PIN);
    //     #endif
...
// Ensure all the conditions are ok for powering
    ...
    if (ITSDK_DRIVERS_GNSS_QUECTEL_L86_FORCEON_PIN != __LP_GPIO_NONE || ITSDK_DRIVERS_GNSS_QUECTEL_L8X_POWERON_PIN != __LP_GPIO_NONE)
    {
        if (ITSDK_DRIVERS_GNSS_QUECTEL_L86_FORCEON_PIN != __LP_GPIO_NONE)
        {
            ...
            LL_GPIO_SetOutputPin(ITSDK_DRIVERS_GNSS_QUECTEL_L86_FORCEON_BANK, ITSDK_DRIVERS_GNSS_QUECTEL_L86_FORCEON_PIN);
        }
        if (ITSDK_DRIVERS_GNSS_QUECTEL_L8X_POWERON_PIN != __LP_GPIO_NONE)
        {
            gpio_configure(ITSDK_DRIVERS_GNSS_QUECTEL_L8X_POWERON_BANK, ITSDK_DRIVERS_GNSS_QUECTEL_L8X_POWERON_PIN, GPIO_OUTPUT_PP);
...
{
    char cmd[DRIVER_GNSS_QUECTEL_CMD_MAXZ];
    if (__quectel_status.isRunning == 1)
...
static gnss_ret_e __quectelNMEA(gnss_data_t *data, uint8_t *line, uint16_t sz, gnss_nmea_driver_t *driver)
{
            ...
            if (line[5] == '0')
            {
                if (line[6] == '0')
                {
                    if (line[7] == '1')
                    {
                        ...
                        if (nmea_getDecimalField(pt, (uint16_t *)&__quectel_status.lastAckedCode) == GNSS_INVALIDFORMAT)
                            return GNSS_INVALIDFORMAT;
                        if (nmea_goNextField(&pt) != GNSS_SUCCESS)
                            return GNSS_INVALIDFORMAT;
                        uint16_t reason;
                        if (nmea_getDecimalField(pt, &reason) == GNSS_INVALIDFORMAT)
                            return GNSS_INVALIDFORMAT;
                        if (reason == 3)
                        {
                            __quectel_status.hasAckedSuccess = 1;
    ...
    case GNSS_SUCCESS:
        ...
        if (driver->triggeringMessage != NMEA_NONE && driver->currentMessage == driver->triggeringMessage && driver->numOfMessages == driver->numOfMessCurShot)
        {
