...
#define DEGTODISTANCE (111139.e-7f)

static int32_t initLat = -1, initLon = -1;
static int32_t lastLat = -1, lastLon = -1;
...
{
    char buff[200];
    char NMEAdata[NMEA_GPRMC_SENTENCE_SIZE];
    ...
    //             {
    //                 if (c->callback != NULL)
    //                     c->callback(triggers & c->triggerMask, &__gnss_config.data, _duration);
    //             }
    //             c = c->next;
    //         }
    //     }
...
{
    if (!__gnss_config.setupDone)
        return GNSS_NOTREADY;
    if (__gnss_config.isRunning)
        return GNSS_ALLREADYRUNNNING;
    if (fixFreq != 0 && fixFreq != 1 && fixFreq != 100)
    ...
    gnss_eventHandler_t *handler)
{
    ...
    if (gnss_isTriggerCallBack(handler) == true)
        return GNSS_ALLREADYREGISTER;
...
static gnss_ret_e __gnss_onDataRefreshed(void)
{
    ...
    if (__gnss_config.data.lastRefreshS > 0)
    {
        ...
        if (f->fixType >= GNSS_FIX_2D)
        {
            ...
            if (f->gps.hdop > 0 && f->gps.hdop < f->fixHdop)
                f->fixHdop = f->gps.hdop;
#endif
#if ITSDK_DRIVERS_GNSS_WITHGLOSAT == __ENABLE
            if (f->glonass.hdop > 0 && f->glonass.hdop < f->fixHdop)
                f->fixHdop = f->glonass.hdop;
#endif
#if ITSDK_DRIVERS_GNSS_WITHGALSAT == __ENABLE
            if (f->galileo.hdop > 0 && f->galileo.hdop < f->fixHdop)
                f->fixHdop = f->galileo.hdop;
#endif
            if (f->fixHdop == 0xFFFF)
                f->fixHdop = 0;
            ...
            if (f->fixHdop > 0)
            {
                if (f->fixHdop < 150)
                    if (f->fixHdop < 200)
                        triggers |= GNSS_TRIGGER_ON_HDOP_200;
                if (f->fixHdop < 400)
                    triggers |= GNSS_TRIGGER_ON_HDOP_400;
                if (f->fixHdop < 800)
                    triggers |= GNSS_TRIGGER_ON_HDOP_800;
            }

            if (f->speed_kmh > 0)
            {
                if (f->speed_kmh >= 5)
                    triggers |= GNSS_TRIGGER_ON_SPEED_5;
                if (f->speed_kmh >= 10)
                    triggers |= GNSS_TRIGGER_ON_SPEED_10;
                if (f->speed_kmh >= 20)
                    triggers |= GNSS_TRIGGER_ON_SPEED_20;
                if (f->speed_kmh >= 40)
                    triggers |= GNSS_TRIGGER_ON_SPEED_40;
                if (f->speed_kmh >= 70)
                    triggers |= GNSS_TRIGGER_ON_SPEED_70;
                if (f->speed_kmh >= 90)
                    triggers |= GNSS_TRIGGER_ON_SPEED_90;
                if (f->speed_kmh >= 110)
                    triggers |= GNSS_TRIGGER_ON_SPEED_110;
                if (f->speed_kmh >= 150)
                    triggers |= GNSS_TRIGGER_ON_SPEED_150;
            }
        }
...
// Manage the Indoor / Outdoor situation with the number of visible sat & signal
        ...
        if (d->satDetailsUpdated == 1)
        {
            ...
            else
            {
                ...
                {
                    if ((_now - d->sat_gps[i].updateTime) < 10 && d->sat_gps[i].signal >= 28)
                        goodSats++;
                }
#endif
#if ITSDK_DRIVERS_GNSS_WITHGLOSAT == __ENABLE
                ...
                {
                    if ((_now - d->sat_glonas[i].updateTime) < 10 && d->sat_glonas[i].signal >= 28)
                        goodSats++;
                }
#endif
#if ITSDK_DRIVERS_GNSS_WITHGALSAT == __ENABLE
                ...
                {
                    if ((_now - d->sat_galileo[i].updateTime) < 10 && d->sat_galileo[i].signal >= 28)
                        goodSats++;
                }
#endif
                if (goodSats > 4)
...
void gnss_processString(char *buff)
{
    if (__gnss_config.withNmeaDecodeur && __gnss_config.driver.nmea.nmeaParser != NULL)
    {
        __gnss_config.driver.nmea.nmeaParser(&__gnss_config.data, buff, strlen(buff), &__gnss_config.driver.nmea);
    }
    __gnss_config.pBuffer = 0;
}
...
void gnss_printState(void)
{
    ...
    if (__gnss_config.data.fixInfo.fixType >= GNSS_FIX_2D)
    {
        if (initLat == -1 || initLon == -1)
        {
            initLat = __gnss_config.data.fixInfo.latitude;
            initLon = __gnss_config.data.fixInfo.longitude;
        }
        lastLat = __gnss_config.data.fixInfo.latitude;
        lastLon = __gnss_config.data.fixInfo.longitude;
...
void gnss_printToTFT(void)
{
    ...
    if (__gnss_config.data.fixInfo.fixType >= GNSS_FIX_2D)
    {
        sprintf(buf, "dLat %d ", (lastLat - initLat));
        ST7735_WriteString(0, 50, buf, Font_7x10, WHITE, BLACK);
        sprintf(buf, "dLon %d ", (lastLon - initLon));
        ST7735_WriteString(0, 60, buf, Font_7x10, WHITE, BLACK);
        sprintf(buf, "dxLat %f ", (float)(lastLat - initLat)*DEGTODISTANCE);
        ST7735_WriteString(0, 70, buf, Font_7x10, WHITE, BLACK);
        sprintf(buf, "dxLon %f ", (float)(lastLon - initLon)*DEGTODISTANCE);
        ST7735_WriteString(0, 80, buf, Font_7x10, WHITE, BLACK);
