...
static void gpioWaitForHigh(GPIO_TypeDef *gpio, uint16_t pin, uint16_t timeout_us)
{
  ...
  while (LL_GPIO_IsOutputPinSet(gpio, pin) && (xTaskGetTickCount() - start <= timeout_us))
  {
  }
}
...
static void i2cdrvStartTransfer(I2cDrv *i2c)
{
  ...
  if (i2c->txMessage.direction == i2cRead)
  {
    ...
    LL_DMA_ConfigAddresses(SENSOR_DMA, SENSOR_DMA_STREAM_TX, (uint32_t)i2c->txMessage.buffer, (uint32_t)LL_I2C_DMA_GetRegAddr(SENSOR_I2C), LL_DMA_GetDataTransferDirection(SENSOR_DMA, SENSOR_DMA_STREAM_TX));
    LL_DMA_SetDataLength(SENSOR_DMA, SENSOR_DMA_STREAM_TX, i2c->txMessage.messageLength);
